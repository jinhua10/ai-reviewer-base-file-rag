...
| 策略 | 适用场景 | 性能 | 复杂度 | 依赖 | 特殊能力 |
|------|---------|------|--------|------|---------|
| **JSON_FILE** | 小规模（<10K类型） | ⭐⭐⭐ | ⭐ | 无 | 分片存储 |
| **H2** | 中等规模（<100K类型） | ⭐⭐⭐⭐ | ⭐⭐ | H2库 | SQL查询 |
| **SQLITE** | 中等规模 | ⭐⭐⭐⭐ | ⭐⭐ | SQLite库 | 单文件DB |
| **REDIS** | 大规模（>100K类型） | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | Redis服务 | 内存缓存 |
| **MONGODB** | 海量数据（>1M类型） | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | MongoDB服务 | 文档存储 |
| **ELASTICSEARCH** | 海量数据+全文搜索 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ES服务 | **全文搜索** |
| **HYBRID** | 生产环境推荐 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | Redis+文件 | 双层缓存 |
| **MEMORY** | 测试环境 | ⭐⭐⭐⭐⭐ | ⭐ | 无（不持久化） | 最快 |
